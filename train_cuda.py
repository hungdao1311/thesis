...
logging.basicConfig(filename='train_company_kim',
                    filemode='a',
...
def run_strong_classifier_multi(samples: np.ndarray, weak_classifiers, threshold):
    ...
    for c in weak_classifiers:
        a = np.array([sample[c['feature']].reshape((1, 72)) for sample in samples]).reshape((len(samples), 72))
...
def build_strong_classifier(samples_file, labels_file, train_param, train_status=TrainStatus.Running_normally):
    ...
    if train_status == TrainStatus.Running_normally:
        ...
        file_name = 'model_joint' + '_curr_' + str(datetime.datetime.now().date()) + '.sav'
        layers = load_model(file_name)
        num_layer = len(layers)
        samples = 'samples_joint_curr'
        labels = 'labels_joint_curr.txt'
        samples = np.load(samples_file + '.npz')['arr_0']
        labels = np.loadtxt(labels_file)
        ori_num_samples = 5640
        saved_param = load_param()
        logger.info(f'Load successfully {saved_param}')
        D_layer = saved_param.D_layer
        F_layer = saved_param.F_layer
        if num_layer == samples[-1][0][0][0] and num_layer == labels[-1]:
            samples = samples[:-1]
            labels = samples[:-1]
            ...
            logger.info('Saved data failed at previous training')
            logger.info('Start reloading data')
    ...
    while F_layer > F_target:
        ...
        while fi > f_max:
            ...
            logger.info(f'Running weak classifier {curr_wae_clf_index} index {feature_idx} with error {err} and alpha {alpha}')
            logger.info(f'Samples length: {len(samples)}')
            sum_alphas += alpha
            threshold = 0.5 * sum_alphas
            logger.info(f'alpha {alpha} --- threshold {threshold}')
            wrong_samples = list()
            wrong_pos_samples = list()
            for i in range(num_samples):
                ...
                if labels[i] == 1 and sample_pred != labels[i]:
                    wrong_pos_samples.append(i)
            ...
            logger.info(f'Wrong pos images {np.array(wrong_pos_samples)}')
            logger.info(f'Wrong images {real_wrong_img}')
            # np.savetxt('/home/administrator/graduate-thesis-develop/graduate-thesis-develop/wrong_img_pred' + str(num_layer) + '_' + str(
            #     len(weak_cls_ensemble)) + '.txt', real_wrong_img, fmt='%f')
        ...
        samples_file = 'samples_joint_curr'
        labels_file = 'labels_joint_curr.txt'
        shape = (1, 1176, 2, 36)
...
def main():
    ...
    logger.info("Start extracting Joint Hog")
    # Pos train set
    ...
    samples_file = 'samples2820'
    labels_file = 'labels2820.txt'
...
def store_neg_image(neg_samples_path=DEFAULT_NEG_DIR, stored_path=DEFAULT_NEG_HOG_FEATURE_DIR, start=0, end=0):
    ...
    logger.info("Start extracting Joint Hog for negative samples")
    count = 537554
    for index in range(start, end):
...
def train(num_training_times, train_param):
    ...
    for index in range(num_training_times):
        ...
        samples_file = 'samples5640'
        labels_file = 'labels5640.txt'
        # samples_file, labels_file = main()
        model = build_strong_classifier(samples_file, labels_file, train_param[index % num_training_times], TrainStatus.Running_after_reset)
...
def load_param():
    with open('saved_param.pkl', 'rb') as input_param:
...
if __name__ == '__main__':
    ...
    # train(1, train_param)
    # store_neg_image()
    # store_neg_image(neg_samples_path='D:\Project\Python\data_set\\add_neg_set_2', start=358527, end=537555)
    store_neg_image(neg_samples_path='//DOMINIK-PC/kim/data/add_neg_set_3', start=537554, end=1085813)
    # main()
