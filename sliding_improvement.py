...
from final_predict import predict_multi
import argparse
import time
import cv2
...
stored_path = 'D:\Project\Python\data_set\improve_true_res'
result = 'D:\Project\Python\graduate-thesis\\final_output'
...
for resized_image in pyramid(image, scale):
    ...
    for (x, y, window) in sliding_window(resized_image, stepSize=16, windowSize=(winW, winH)):
...
# Predict all window
lst_indexPositive = predict_multi(lst_img, model_path)
...
for i in lst_indexPositive:
    ...
    ori_x, ori_y, new_winW, new_winH = reverse_window(subX, subY, subImg.shape[1], subImg.shape[0],
                                                      scale ** subLevel, image.shape[1], image.shape[0])
