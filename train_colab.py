...
DEFAULT_NEG_DIR = '/content/additional_neg_set'
DEFAULT_NEG_HOG_FEATURE_DIR = '/content/gdrive/My Drive/Graduate Thesis People counting/negative_joint_hog'
    ...
    for i in range(num_samples):
        ...
        if sample_pred == 1:
            ...
            if sample_pred == labels[i]:
                ...
                false_neg += 1
    return false_pos, true_pos, false_neg, true_neg
...
def explore_weak_clf(samples, labels, num_pos, num_neg, weights, weights_ada, train_param, sam_ori_index):
    ...
    samples, labels, weights, weights_ada, sam_ori_index = utils.shuffle_data(samples, labels, weights, weights_ada, sam_ori_index)
    count = 1
    for idx in selected_feature:
        ...
        for index in range(len(samples_feature)):
            pred = svm_clf.predict(samples_feature[index].reshape((1, 72)))[0]
...
def build_strong_classifier(samples_file, labels_file, train_param, train_status=0):
    F_target = 0.000001
    saved_param = dict()
    if train_status == 0:
        num_layer = 0
    ...
    while F_layer > F_target:
        ...
        if train_status == 0:
            ...
            logging.info("Load data completed")
            num_samples = len(samples)
            weights_ada = np.loadtxt('weights_ada.txt')
            logging.info(f"Staring training on {num_samples} records on layer {num_layer}")
            fi = saved_param['fi']
            weak_cls_ensemble = saved_param['weak_cls_ensemble']
            sum_alphas = 0
            for c in weak_cls_ensemble:
                sum_alphas += c['alpha']
            train_status = 0
        ...
        while fi > f_max:
            ...
            num_samples = len(samples)
            logging.info(f"Run strong classifier on {int(ori_num_samples/2 + curr_neg_count)} samples with {len(weak_cls_ensemble)} weak clf")

            # Running strong classifier on whole dataset
            false_pos, true_pos, false_neg, true_neg = clf_on_strong_clf(samples, labels, weak_cls_ensemble, threshold, sam_ori_idx)
            ...
            while not np.isclose((upper_bound - lower_bound), 0):
                ...
                # Running strong classifier on whole dataset
                false_pos, true_pos, false_neg, true_neg = clf_on_strong_clf(samples, labels, weak_cls_ensemble, threshold)
                d = true_pos / (true_pos + false_neg)
                fi = false_pos / (false_pos + true_neg)
            ...
            if num_layer > len(layers):
                layers.append(Layer.Layer(threshold, weak_cls_ensemble))
            else:
                layers[-1] = Layer.Layer(threshold, weak_cls_ensemble)
            save_model(layers)
            saved_param = {
                'fi': fi,
                'd': d,
                ...
                'weak_cls_ensemble': weak_cls_ensemble
            }
            saved_param(saved_param)
            samples_file = 'samples_curr'
            labels_file = 'labels_curr.txt'
            weights_file = 'weight_ada.txt'
            np.savez_compressed(samples_file, samples)
            np.savetxt(labels_file, labels, fmt='%f')
            np.savetxt(weights_file, weights_file, fmt='%f')

        F_layer = F_layer * fi
        D_layer = D_layer * d_min
...
def main():
    ...
    for index in range(len(pos_list))[0:1500]:
    ...
    samples_file = 'samples3000'
    labels_file = 'labels3000.txt'
    np.savez_compressed(samples_file, samples)
    np.savetxt(labels_file, labels, fmt='%f')
...
def store_neg_image(neg_samples_path=DEFAULT_NEG_DIR, stored_path=DEFAULT_NEG_HOG_FEATURE_DIR, start=0, end=0):
    ...
    for index in range(start, end):
        ...
        logging.info(f'Stored finished {count}')
        count += 1
...
def save_model(model):
    model_save_name = 'model.sav'
    path = F"/content/gdrive/My Drive/Graduate Thesis People counting/{model_save_name}"
...
def train(num_training_times, train_param):
    ...
    for index in range(num_training_times):
        ...
        samples_file = '/content/gdrive/My Drive/Graduate Thesis People counting/samples'
        labels_file = '/content/gdrive/My Drive/Graduate Thesis People counting/labels.txt'
        # samples_file = '/content/samples200'
        # labels_file = '/content/labels200.txt'
        # samples_file, labels_file = main()
        model = build_strong_classifier(samples_file, labels_file, train_param[index % num_training_times])
        file_name = 'model_' + str(index + start) + '_' + str(train_param[index % num_training_times]['f_max']) + '_' + str(
...
if __name__ == '__main__':
    ...
    # hog_feature = create_roi_hog(image, scale=(2, 1))
    # logging.info(hog_feature)
    train_param = [
        {'f_max': 0.65, 'C': [1]},
    ]
    train(1, train_param)
    # store_neg_image(neg_samples_path='/content/additional_neg_set')
    # main()
